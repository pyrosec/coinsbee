...
class CoinsbeeClient extends base_puppeteer_1.BasePuppeteer {
    ...
    async _buy({ products, name, value }) {
    ...
    async checkout({ pay, coin = 'ETH' }) {
        ...
        const response = await this._page.evaluate(() => {
            ...
            return {
                ...
                address: document.querySelector('div.infos').innerText.match(/0x[a-fA-F0-9]{40}/g)[0]
            };
        });
        if (pay) {
            const wallet = this._getWallet();
    ...
    async login({ email, password }) {
        ...
        await this.timeout({ n: 1000 });
        if (await this._page.evaluate(() => Boolean(document.querySelector('div.alert.alert-danger')))) {
