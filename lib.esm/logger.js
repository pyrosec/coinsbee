...
import { createLogger as createWinstonLogger, transports, format, addColors, } from "winston";
const customLevels = {
    error: 0,
    warn: 1,
    data: 2,
    ...
    const logger = createWinstonLogger({
        ...
        transports: [
            new transports.Console({
                level: "verbose",
                format: format.combine(format.label({ label: proc }), format.timestamp(), format.printf(customFormatter)),
            }),
        ],
    });
    return logger;
