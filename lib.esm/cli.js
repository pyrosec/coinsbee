...
export async function saveSession(coinsbee, json = false, filename = "session.json") {
    await mkdirp(path.join(process.env.HOME, ".coinsbee"));
    await fs.writeFile(path.join(process.env.HOME, ".coinsbee", filename), coinsbee.toJSON());
    if (!json)
        logger.info("saved to ~/" + path.join(".coinsbee", filename));
}
...
export async function loadSession() {
    const proxyOptions = await loadProxy();
    const coinsbee = CoinsbeeClient.fromJSON(await fs.readFile(path.join(process.env.HOME, ".coinsbee", "session.json")));
...
export async function setProxy(proxyUri) {
    await mkdirp(path.join(process.env.HOME, ".coinsbee"));
...
export async function loadProxy() {
    await mkdirp(path.join(process.env.HOME, ".coinsbee"));
    try {
        return proxyStringToObject(await fs.readFile(path.join(process.env.HOME, ".coinsbee", "proxy"), "utf8"));
    }
    catch (e) {
        return null;
    }
...
export async function loadSessionFrom(name) {
    const coinsbee = CoinsbeeClient.fromObject(JSON.parse(await fs.readFile(path.join(process.env.HOME, ".coinsbee", name), "utf8")));
    await saveSession(coinsbee);
}
